...
export default function RecipesVideos() {
  ...
  async function fetchData(cat_id = '', page = 1, search = '') {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_Backend_API_URL}/api/listRecipeVideo?limit=6&categoryId=${cat_id}&page=${page}&search=${encodeURIComponent(search)}`,
        {
          method: "GET",
          headers: {
                      ...
                      <div className="w-1/3 px-2" key={index}>
                        <div className="product-card">
                          ...
                          <div className="content">
                            <h3>
                              <Link href={`/recipes-videos/${item.video_recipe_slug}`}>
                                {item.video_recipe_name.length > 30 ?
                                  item.video_recipe_name.substring(0, 30) + '...' :
                                  item.video_recipe_name}
                              </Link>
                            </h3>
                            <span>${item.video_recipe_Price}</span>
                          </div>
                        </div>
                      </div>
                    ))
