...
export default function single_video({ recipe_video_slug }) {

    const [data, setData] = useState([]); // Initialize with default values
        ...
        async function fetchData() {
            ...
            try {
                const response = await fetch(
                    `${process.env.NEXT_PUBLIC_Backend_API_URL}/api/singleRecipeVideo/${recipe_video_slug}`,
                    { method: "GET" }
                );
                ...
                <div className="singlePDF flex">
                    <div className="w-1/2">
                        <div className="shop-details-img">
                            <img
                                src={`${process.env.NEXT_PUBLIC_Backend_API_URL}/${data.video_recipe_thum_image}`}
                                alt="Image"
                            />
                        </div>
                    </div>
                    <div className="w-1/2">
                        <div className="shop-desc pl-20">
                            <h1>{data.video_recipe_name}</h1>
                            <div className="price">
                                <span className="new-price">${data.video_recipe_Price}</span>
                            </div>
